/**
 * Trello integration plugin for Franz
 *
 * @summary     Integrates Trello into the Franz application
 * @since       1.0.0
 */
...
let aboutScripts = {
	...
	renderDialog : function() {
		...
		// Header
		let aboutHeaderBox = '<div class="pop-over-header"><span class="pop-over-header-title">Trello Franz Integration</span></div>';
		...
		let aboutInfoVersion = '<div class="franz-about-version"><span>Version ' + currentVer + '</span><a href="https://github.com/extendfranz/trello/blob/' + currentVer + '/CHANGELOG.md" target="_blank">What\'s New</a></div>';
		let aboutInfoUpdate = '<div class="franz-about-update">' + preUpdate + '</div>';
		let aboutInfoWrapperAfter = '</div><div class="clearfix"></div>';

		// Details box
		let aboutDetailsBox = '<hr /><div class="franz-about-details"><p>The Trello integration for Franz is designed by <a href="http://extendfranz.com" target="_blank">ExtendFranz</a> and is released under the MIT license.</p><p>Questions? Comments? <a href="https://github.com/extendfranz/trello/issues" target="_blank">Let us know</a>!</p><p>Want to help? <a href="http://extendfranz.com/donate/" target="_blank">Make a donation</a> or <a href="https://github.com/extendfranz/trello/issues" target="_blank">get involved</a>!</p></div>';

		// Footer box
		let aboutFooterBox = '<hr /><div class="franz-about-footer">Franz and the Franz logo are copyright Stefan Malzner.<br />This plugin is not affiliated with or endorced by Franz in any way.</div>';
...
function updateCheck() {
	let latestVer = readCookie('franz_trello_latest_version');

	if(latestVer === null) {
		$.get('https://raw.githubusercontent.com/extendfranz/trello/master/package.json', function(data) {
			let packagedata = $.parseJSON(data);

			if (packagedata) {
				latestVer = packagedata.version;
				
				createCookie('franz_trello_latest_version', latestVer, 24);
				
				updateDisplay(latestVer);
			} else {
				eraseCookie('franz_trello_latest_version');
